...
try:
    ...
    BallVelocityEstimator = None
    ImpactType = None

COLLECT_DATA = True


def load_or_select_fixed_keypoints(video_path: str):
    if (
        FIXED_COURT_KEYPOINTS_LOAD_PATH is not None
        and Path(FIXED_COURT_KEYPOINTS_LOAD_PATH).is_file()
    ):
    ...
    image = next(first_frame_generator)
    selected_keypoints = []
    ...
    if len(selected_keypoints) < 4:
        raise ValueError(
            f"Expected at least 4 court keypoints, but got {len(selected_keypoints)}."
        )

    if len(selected_keypoints) > 12:
        selected_keypoints = selected_keypoints[:12]
...
if upload_video or st.session_state["video"] is not None:
    ...
    if st.session_state["df"] is None:
        ...
        with st.spinner("Analysing video ..."):
            ...
            SELECTED_KEYPOINTS = load_or_select_fixed_keypoints("tmp.mp4")
    ...
    estimate_velocity = st.checkbox(
        "Calculate Ball Velocity",
        disabled=BallVelocityEstimator is None,
    )
    if BallVelocityEstimator is None:
        st.info("Ball velocity estimation is unavailable because `estimate_velocity.py` is missing.")

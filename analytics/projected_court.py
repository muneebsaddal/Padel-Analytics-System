...
class ProjectedCourt:
    ...
    def homography_matrix(self, keypoints_detection: Keypoints) -> np.ndarray:
        ...
        number_keypoints_detected = len(keypoints_detection)

        if number_keypoints_detected >= 12 and number_keypoints_detected not in (18, 22):
            # Fixed/manual selection can include extra clicks. Use the first 12
            # canonical court points so projection still works.
            keypoints_detection = keypoints_detection[:12]
            number_keypoints_detected = len(keypoints_detection)

        if 4 <= number_keypoints_detected <= 12:
            ...
            dst_keypoints = self.court_keypoints.keypoints(number_keypoints=12)[
                :number_keypoints_detected
            ]
        ...
        elif number_keypoints_detected == 18:
            src_keypoints = keypoints_detection
            # Court keypoints of the given frame
            src_points = np.array(
                [
        ...
        elif number_keypoints_detected == 22:
            src_keypoints = keypoints_detection
            # Court keypoints of the given frame
            src_points = np.array(
                [
            ...
            raise ValueError(
                f"Unhandled number of keypoints detected: {number_keypoints_detected}. "
                "Expected 4 to 12, 18, or 22 detected keypoints."
            )
